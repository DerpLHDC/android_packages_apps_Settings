...
/**
 * Dialog preference controller to set the Bluetooth A2DP config of LHDCV5 lossless raw mode enabled/disabled
 */
public class BluetoothLHDCAudioLosslessRawDialogPreferenceController extends
    ...
    private static final long LHDCV5_FEATURE_MASK = 0xFF000000;
    private static final long LHDCV5_FEATURE_TAG = 0x5C000000;
    private static final long LHDCV5_LOSSLESS_FEATURE = 0x80;
    private static final long LHDCV5_LOSSLESS_RAW_FEATURE = 0x100;
    ...
    protected void writeConfigurationValues(final int index) {
        synchronized (mBluetoothA2dpConfigStore) {
            ...
            codecSpecific3Value &= ~ LHDCV5_FEATURE_MASK;
            codecSpecific3Value |= LHDCV5_FEATURE_TAG;
            if (index != 0) {
                codecSpecific3Value |= LHDCV5_LOSSLESS_RAW_FEATURE;
            } else {
                codecSpecific3Value &= ~LHDCV5_LOSSLESS_RAW_FEATURE;
            }
    ...
    public void updateState(Preference preference) {
        ...
        synchronized (mBluetoothA2dpConfigStore) {
            long codecSpecific3Value = mBluetoothA2dpConfigStore.getCodecSpecific3Value();
            long tmp = codecSpecific3Value & LHDCV5_FEATURE_MASK;
            long inx = codecSpecific3Value & LHDCV5_LOSSLESS_FEATURE;

            if (currentConfig != null && 
                currentConfig.getCodecType() == BluetoothCodecConfig.SOURCE_CODEC_TYPE_LHDCV5) {
                if (tmp == LHDCV5_FEATURE_TAG && inx != 0) {
                    // lossless raw mode requires lossless is enabled first
    ...
    int convertCfgToBtnIndex(long tag, long index) {
        int ret = 0;
        long tmp = index & LHDCV5_FEATURE_MASK;
        if (tmp == tag) {
            if ((index & LHDCV5_LOSSLESS_RAW_FEATURE) != 0) {
                ret = 1;
            } else {
                ret = 0;
            }
