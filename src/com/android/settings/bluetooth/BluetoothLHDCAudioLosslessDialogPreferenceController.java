...
/**
 * Dialog preference controller to set the Bluetooth A2DP config of LHDCV5 lossless enabled/disabled
 */
public class BluetoothLHDCAudioLosslessDialogPreferenceController extends
    ...
    protected void writeConfigurationValues(final int index) {
        synchronized (mBluetoothA2dpConfigStore) {
            ...
            if (index != 0) {
                codecSpecific3Value |= LHDCV5_LOSSLESS_FEATURE;
            } else {
                codecSpecific3Value &= ~LHDCV5_LOSSLESS_FEATURE;
            }
    ...
    int convertCfgToBtnIndex(long tag, long index) {
        ...
        if (tmp == tag) {
            if ((index & LHDCV5_LOSSLESS_FEATURE) != 0) {
                ret = 1;
            } else {
                ret = 0;
            }
